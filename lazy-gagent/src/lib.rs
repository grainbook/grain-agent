...
//!
//! This crate sits **on top of** the engine and provides three
//! mutating operations:
//!
//! - [`install`] — append a `[[plugin]]` block to `plugin-spec.toml`
//!   then sync (git clone / local symlink) the new entry.
//! - [`update`] — `git pull` on git-sourced plugins; no-op for local
//!   symlinks (source tree is live).
//! - [`remove`] — drop the `[[plugin]]` block from the spec and
//!   optionally `rm -rf` (or `unlink`) the installed directory.
//!
//! These are deliberately Rust-side primitives: they manipulate the
//! file system and spawn `git`. The TUI surfaces them as slash
//! commands (`/install`, `/update`, `/remove`); a future Boa script
//! could expose them as agent-callable tools by wrapping these
//! functions behind a JS shim.
//!
//! # Bootstrap story
//!
//! `lazy-gagent` itself is just another plugin under
//! `<workspace>/.grain/plugins/lazy-gagent/`. The engine pulls it in
//! from `plugin-spec.toml` before discovery runs, so there's no
//! chicken-and-egg: the manager and the things it manages live in
...
pub struct InstallOutcome {
    ...
    /// freshly-added entry should appear in `installed` on success or
    /// `failed` on a clone / symlink error.
...
/// - URL-like → git clone
/// - filesystem path → symlink (relative paths anchored at the spec
...
/// plugin was sourced:
///
/// - **Symlink** (local source) — no-op. The link already points at a
///   live source tree; the user is the one editing it.
...
///
/// `delete_files`:
/// - `false` (default) — only edit the spec file. Useful when the
///   user wants to keep their local edits around in
///   `<plugins_dir>/<name>/` but stop the engine from auto-loading.
...
mod tests {
    ...
    fn install_appends_to_spec_and_syncs_local_source() {
        ...
        // Source plugin with a minimal manifest.
        ...
        // Spec file now mentions the new plugin.
        let parsed = load_plugin_spec(&spec_path).unwrap();
        assert_eq!(parsed.plugins.len(), 1);
        assert_eq!(parsed.plugins[0].name, "source-plugin");
    ...
    fn install_rejects_bad_names() {
        ...
        for bad in ["", "a/b", "..", "..foo"] {
            let err = install(&spec_path, &plugins_dir, bad, "/whatever", None).unwrap_err();
    ...
    fn remove_drops_spec_entry_and_keeps_files_by_default() {
        ...
        // Pretend the install directory survived a previous boot.
    ...
    fn remove_unlinks_symlink_without_touching_source() {
        ...
        // Install via the public API so we get a real symlink.
    ...
    fn remove_refuses_unknown_name() {
        ...
        let err = remove(&spec_path, &plugins_dir, "y", false).unwrap_err();
    ...
    fn update_returns_not_found_for_missing_plugin() {
        ...
        let err = update(&plugins_dir, "nope").unwrap_err();

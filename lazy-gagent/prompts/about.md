...
Phase C-0 (`plugin.toml` declarative install) is already shipped in the engine. Phase C-1 will fill in this directory with:

- `scripts/install.js` — a Boa-loaded tool the agent can call: `lazy_gagent.install(name, src)` appends a `[[plugin]]` block to `plugin.toml` and runs sync.
...
Until those land, treat this plugin as documentation: when a user asks "how do I install plugin X" the answer is "edit `<workspace>/.grain/plugin.toml` and add a `[[plugin]]` entry — engine syncs at next launch".
